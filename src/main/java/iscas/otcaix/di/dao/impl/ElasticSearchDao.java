...
public class ElasticSearchDao {
	...
	public void aggregationQuery() throws Exception {
		SearchResponse sr = client.prepareSearch().setQuery(QueryBuilders.matchAllQuery())
				.addAggregation(AggregationBuilders.terms("1").field("type"))
				// .addAggregation(
	...
	public void mutilCombineQuery() {
		...
		SearchRequestBuilder srb1 = client.prepareSearch()
				.setQuery(QueryBuilders.queryStringQuery("eng").field("address")).setSize(1);
		// 查询请求2//matchQuery
	...
	public void scorllQuery() throws Exception {
		...
		SearchResponse scrollResp = client.prepareSearch("collection1").setSearchType(SearchType.SCAN)
				.setScroll(new TimeValue(60000)).setQuery(queryString).setSize(100).execute().actionGet();
		// 100
		// hits
		// per
		// shard
	...
	public void querySimple() throws Exception {
		...
		SearchResponse sp = client.prepareSearch("users").execute().actionGet();
	...
	public void queryComplex() throws Exception {
		SearchResponse sp = client.prepareSearch("collection1")// 检索的目录
				...
				// .setPostFilter(FilterBuilders.rangeFilter("price").from(10).to(550.23))//过滤
				// --Filter
	...
	public void indexdata() throws Exception {
		...
		XContentBuilder doc = XContentFactory.jsonBuilder().startObject().field("title", "中国").field("price", 205.65)
				.field("type", 2).field("status", true).field("address", "河南洛阳").field("createDate", new Date())
				.endObject();
				// collection为索引库名，类似一个数据库，索引名为core，类似一个表
				// client.prepareIndex("collection1",
				// "core1").setSource(doc).execute().actionGet();

		// 批处理添加
		bulk.add(client.prepareIndex("collection1", "core1").setSource(doc));

		doc = XContentFactory.jsonBuilder().startObject().field("title", "标题").field("price", 251.65).field("type", 1)
				.field("status", true).field("address", "美国东部").field("createDate", new Date()).endObject();
		// collection为索引库名，类似一个数据库，索引名为core，类似一个表
		// client.prepareIndex("collection1",
		// "core1").setSource(doc).execute().actionGet();
		// 批处理添加
		...
		doc = XContentFactory.jsonBuilder().startObject().field("title", "elasticsearch是一个搜索引擎").field("price", 25.65)
				.field("type", 3).field("status", true).field("address", "china").field("createDate", new Date())
				.endObject();
		// collection为索引库名，类似一个数据库，索引名为core，类似一个表
		// client.prepareIndex("collection1",
		// "core1").setSource(doc).execute().actionGet();
